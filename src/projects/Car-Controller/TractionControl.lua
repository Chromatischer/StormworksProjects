...
function TractionControl.update(input, config, state, output)
	...
	-- In 2D local space, if velocity is purely X, beta_v is 0.
	-- If car is sliding, beta_v deviates.
	-- state.driftAngle is the angle between heading and velocity vector.

	local driftQuotient = math.abs(state.driftAngle)
	...
	if config.mode == "Sport" or config.mode == "Drive" then
		if driftQuotient > config.driftThreshold then
			...
			local counterSteer = -state.driftAngle * config.counterSteerGain

			-- Apply to all wheels (simplified offset for now, ideally re-run Ackermann but offset is okay for correction)
			output.steerFL = output.steerFL + counterSteer
			output.steerFR = output.steerFR + counterSteer
			output.steerRL = output.steerRL + counterSteer
			output.steerRR = output.steerRR + counterSteer
	...
	if input.brake > 0.05 then
		local carSpeed = state.forwardSpeed
		local threshold = carSpeed * config.absSlipThreshold -- e.g. 0.8 * speed

		-- Simple per-axle check (assuming average axle speed)
		-- Front Axle
		local frontWheelSpeed = state.wheelRPS_Front * math.pi * config.wheelDiameter -- m/s
		...
		-- Rear Axle
		local rearWheelSpeed = state.wheelRPS_Rear * math.pi * config.wheelDiameter
